...
def getleaderboard(startfrom):
    ...
    query = cursor.execute("SELECT p.id, p.name, p.image_url, ((SELECT SUM(amount) FROM score WHERE profile_id=p.id AND time>=(UNIX_timestamp(timestamp(current_date))+19800))) as score FROM profile AS p ORDER BY score DESC LIMIT "+str(startfrom)+", "+str(startfrom+10))
    result = cursor.fetchall()
    return result
...
def postprofile(name,rollno,phone_no,referal,imageurl):
    ...
    try:
        ...
        query = cursor.execute("INSERT INTO score VALUES(NULL, '"+rollno+"',10,"+str(time.time()+(3600*24*30))+",1),(NULL, '"+referal+"',10,"+str(time.time()+(3600*24*30))+",1)")
